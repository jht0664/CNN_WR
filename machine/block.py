...
# step0: clean up log file to make initial coordinate like:
#	grep "1024 512 512 " make_init.log | awk '{print $4 " "$5}' > target.list
# step1: read list file and split to train, test, and eval sets,
# step2: split training sets with repect to #blocks for training sets. 
...
parser = argparse.ArgumentParser(
	...
	description='wipe out and clean data sets of Widom-Rowlinson model for machine learning with block average')
## args
...
parser.add_argument('-s1', '--select1', default=0.5, nargs='?', type=float, 
	help='select mixed temperature/density1 (< args.select1) for training set')
parser.add_argument('-s2', '--select2', default=1.0, nargs='?', type=float, 
	help='select sepearated temperature/density2 (> args.select2) for training set')
...
parser.add_argument('-nben', '--n_ensembles_train', default=-1, nargs='?', type=int,
        help='# of ensembles to use for train set (-1 means all ensembles are used)')
parser.add_argument('-ne', '--n_eval', default=100, nargs='?', type=int,
	help='# of ensembles for eval set per grid.npy (-1 to use all data)')
...
# check n_ensembles from a input file (grid.npy)
filename = args.input_prefix+"."+str(list_file_idx[0])+".npy"
try:
...
print(" All your data has {} ensembles. If no, please check again.".format(n_sets))
if args.n_ensembles_train == -1 :
        n_ensembles = n_sets
else:
	n_ensembles = min(args.n_ensembles_train, n_sets)
	print(" But, reduced into {}".format(n_ensembles))

if n_ensembles > args.n_eval: # if #ensemble to load as an eval data is less than #ensembles of training data
	if args.n_eval < 0: # negative
		n_eval_ensembles = n_ensembles
		print(" #ensemble for evaluation data = {} same as training set case".format(n_eval_ensembles))
	else:
		n_eval_ensembles = args.n_eval
		print(" #ensemble for evaluation data = {} (< training set case)".format(n_eval_ensembles))
else:
	n_eval_ensembles = n_ensembles
	print(" #ensemble for evaluation data = {} same as training set case".format(n_eval_ensembles))
	print(" your initial args.e_val is wrong!!!!")
...
def make_npy_files_mode(mode, i_block, idx_array, input_prefix, output_prefix):
	...
	n_data = len(idx_array)
	if gen_cat:
		set_coord=np.empty((n_data,n_ensembles*pow(args.n_grids+2,3)))
		set_temp=np.empty((n_data,n_ensembles))
		set_cat=np.empty((n_data,n_ensembles))
		esti_n_sets = n_ensembles
	else: # eval case
		set_coord=np.empty((n_data,n_eval_ensembles*pow(args.n_grids+2,3)))
		set_temp=np.empty((n_data,n_eval_ensembles))
		esti_n_sets = n_eval_ensembles
	print(" collecting sets for {} mode".format(mode))
	# run each sample
	for i_data in np.arange(n_data):
		...
		if gen_cat:
			if esti_n_sets != n_sets:
				print("you may use input data in middle..")
				#raise RuntimeError("#ensembles sizes are different in {} file like {} != {}".format(filename, n_ensembles, n_sets))
		else:
			if esti_n_sets > n_sets:
				raise RuntimeError("#ensembles sizes you asked are less than #sets in {} file like {} > {}".format(filename, n_ensembles, n_sets))
