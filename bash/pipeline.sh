...
search_size=0         # YES=1/NO=0 for copying model.config's
...
1 ) # step1: make super cells and grid interpolation into npy files
	...
	#~/new_WR/script/machine/run_1prep.sh confout.gro $n_files grid 2 $n_grids $n_copies_train
...
2 ) # step2: make blocks for training set
	...
	cd grid
	#run_2block.sh ***** removed
	python $path_py"machine/block.py" -i target.list -ipf grid -s1 $temp1 -s2 $temp2 -prop $proportion_test -nb $n_files_train -seed 1985 -ng $n_grids -nbe $n_files_eval -ne $n_copies_eval
	cd ..
...
3 ) # step3: make machine learning model with some CNN models
	...
	if [ $search_size -eq 1 ]; then
		cp ${path_model}* ./
	elif [ $search_size -eq 0 ]; then
		cp ${path_model}* ./
		sed -i "s#4 16#${repl_text}#g" model.config.*
	else
		echo "something wrong to copy models.config?"
		exit 0
	fi
	# run machin learning
	i_model=$start_model
	while [ $i_model -lt $end_model ]; do
		...
		while [ $i -lt $n_files_train ]; do
			...
			if [ ! -f $out_model ]; then
			        python $path_py"machine/train.py" -i $input_file -it "test" -ng $n_grids -config $config_file -o $out_model
			else
...
5 ) # step5: fit for Tc and make figures for candidate CNN models
	...
	python $path_py"machine/fit_avg.py"
	cd ..
